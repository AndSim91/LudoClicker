...
export function GadgetRhythmGame({
  ...
  const notes = useMemo(
    () => createGadgetRhythmNotes(minigame.seed),
    [minigame.seed],
  );
  ...
  const finishRun = useCallback((forcedScore?: number) => {
    if (completedRef.current) return;
    completedRef.current = true;
    const score = forcedScore ?? calculateGadgetQuality(
  ...
  const registerMisses = useCallback((currentTimeline: number) => {
    let changed = false;
  ...
  const hitLane = useCallback((lane: GadgetLane) => {
    ...
    setLastJudgment(
      timing.judgment === "perfect"
        ? "Perfect"
        : timing.judgment === "good"
          ? "Good"
          : "Almost",
    );
    ...
    return (
      ...
      >
        <section className="gadget-minigame-result">
          ...
          {quality === 0 ? <p className="gadget-result-warning">Il prodotto può essere accettato, ma non venderà finché la qualità resta allo 0%.</p> : null}
          <div className="gadget-result-actions">
            {accepted ? (
              <button type="button" className="primary" onClick={onContinue}>Continua</button>
            ) : (
              <>
  ...
  const countdown = timelineMs < 0
    ? Math.max(1, Math.ceil(-timelineMs / 1_000))
    : 0;
      ...
      <section className="gadget-minigame-shell">
        <header>
          ...
          <div className="gadget-minigame-stats" aria-live="polite">
            ...
            <span>Record combo <strong>{bestCombo}</strong></span>
            <span>Errori <strong>{falseInputs}</strong></span>
        ...
        <div className="gadget-rhythm-board" aria-label="Quattro corsie della prova qualità">
          ...
          {notes.map((note) => {
            ...
            const progress = (timelineMs - (note.targetAtMs - GADGET_MINIGAME_CONFIG.travelMs)) /
              GADGET_MINIGAME_CONFIG.travelMs;
            if (progress < -0.08 || progress > 1.15) return null;
            const top = Math.max(-6, Math.min(94, progress * GADGET_MINIGAME_CONFIG.targetPositionPercent));
            return (
              <button
                type="button"
                className="gadget-falling-note"
                key={note.id}
                tabIndex={-1}
                style={{
          ...
          <div className="gadget-lane-controls">
            {LANE_CONTROLS.map((control, lane) => (
              ...
              >
                <strong>{control.arrow}</strong>
                <span>{control.key}</span>
              </button>
            ))}
          </div>
          {countdown > 0 ? <div className="gadget-countdown" aria-live="assertive">{countdown}</div> : null}
          {lastJudgment && countdown === 0 ? <div className="gadget-judgment" aria-live="polite">{lastJudgment}</div> : null}
        </div>

        <footer>
          <p>Premi la corsia quando la nota raggiunge la linea. Usa frecce o WASD, oppure tocca i pulsanti.</p>
          <button type="button" onClick={() => finishRun(0)}>Abbandona il tentativo</button>
        </footer>

        {paused ? (
          <div className="gadget-minigame-pause" role="alertdialog" aria-modal="true" aria-labelledby="gadget-pause-title">
            <h3 id="gadget-pause-title">{pauseReason}</h3>
            <p>Il conto alla rovescia riprenderà da dove si è fermato.</p>
            <button type="button" className="primary" onClick={resumeRun} autoFocus>Riprendi</button>
          </div>
        ) : null}
      </section>
    </div>
