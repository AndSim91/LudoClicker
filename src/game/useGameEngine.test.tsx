...
describe("useGameEngine pause", () => {
  ...
  it("keeps the game clock and every remaining duration frozen", async () => {
    ...
    expect(
      reloadedWhilePaused.school.nextFeeAt - reloadedWhilePaused.automation.lastProcessedAt,
    ...
    expect(
      result.current.state.school.nextFeeAt - result.current.state.automation.lastProcessedAt,
    ).toBe(remainingMonthMs);
    expect(
  ...
  it("reports a storage failure without pretending the game is saved", () => {
    ...
    expect(result.current.saveStatus.error).toMatchObject({
